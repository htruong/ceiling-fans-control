...
async fn handle_finish(
    ...
    match handler.session {
        ...
        Some(ref mut session) => {
            ...
            // BUGFIX: upstream used `SigningKey::from_bytes` here, which interprets
            // `pairing.public_key` as a *private* seed and derives a fresh keypair
            // — so signature verification silently failed for every pair-verify.
            // The correct API is `VerifyingKey::from_bytes`.
            use ed25519_dalek::Verifier;
            let verifying = ed25519_dalek::VerifyingKey::from_bytes(&pairing.public_key)
                .map_err(|_| tlv::Error::Authentication)?;
            if verifying.verify(&device_info, &device_signature).is_err() {
